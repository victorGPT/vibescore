...
- 每次创建/修改/删除前必须先查阅 `architecture.canvas` 和 `interaction_sequence.canvas`，确认受影响的节点。
- 制定计划前必须先更新 Canvas：运行 `node scripts/ops/architecture-canvas.cjs` 和 `node scripts/ops/interaction-sequence-canvas.cjs`；若脚本不可用，手动更新并保持节点格式与已有节点一致。
