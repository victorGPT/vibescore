...
async function trySyncSnapshot({ baseUrl, userId, leaderboardPublic }) {
  ...
  let avatarUrl = null;
  try {
    const { data } = await serviceClient.database.from("users").select("nickname,avatar_url").eq("id", userId).maybeSingle();
  ...
  for (const w of windows) {
    try {
      await serviceClient.database.from("vibeusage_leaderboard_snapshots").update({ display_name: displayName, avatar_url: nextAvatarUrl }).eq("period", w.period).eq("from_day", w.from).eq("to_day", w.to).eq("user_id", userId);
    } catch (_err) {
    }
  }
}
